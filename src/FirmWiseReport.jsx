...
const columns = [
  ['dealingPerson', 'Salesman'], ['company', 'Company'], ['target', 'Sales Target'],
...
export default function FirmWiseReport({ firms }) {
  ...
  const [dealingPerson, setDealingPerson] = useState(''), [report, setReport] = useState(null)
  const [people, setPeople] = useState({ dealing: [] }), [sort, setSort] = useState({ key: 'company', direction: 1 })
  ...
  const load = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ firm, asOfDate }); if (dealingPerson) params.set('dealingPerson', dealingPerson)
      const data = await api(`/api/reporting/firm-wise?${params}`); setReport(data)
      if (!dealingPerson) setPeople({ dealing: unique(data.rows.map((row) => row.dealingPerson)) })
      setStatus((current) => ({ ...current, loading: false }))
    } catch (error) { setStatus({ loading: false, refreshing: false, error: error.message, message: '' }) }
  }, [firm, asOfDate, dealingPerson])
  useEffect(() => { load() }, [load])
  ...
  async function exportExcel() {
    ...
    sheet.addRows([[`${firm} - Firm Wise Report`], [`As of ${asOfDate}`, `FY ${report.financialYear}`], [], columns.map(([, label]) => label), ...rows.map((row) => columns.map(([key]) => row[key] ?? '')), ['TOTAL', '', report.totals.target, ...columns.slice(3).map(([key]) => report.totals[key] ?? '')]])
    sheet.mergeCells('A1:M1'); sheet.getRow(1).font = { bold: true, size: 15 }; sheet.getRow(4).font = { bold: true, color: { argb: 'FF000000' } }; sheet.getRow(4).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF20DCE0' } }; sheet.views = [{ state: 'frozen', xSplit: 2, ySplit: 4 }]
    sheet.columns = columns.map(([, label], index) => ({ key: label, width: index === 1 ? 36 : 18 })); for (let row = 5; row <= sheet.rowCount; row += 1) for (let col = 3; col <= 13; col += 1) sheet.getCell(row, col).numFmt = col === 8 ? '0.0"%"' : '#,##0.00'
  ...
  return <section className="stack firm-wise-report"><div className="master-title"><div><h2>FIRM WISE REPORT</h2><p>Sales, collections, targets and receivable ageing as of {asOfDate}.</p></div><div className="button-row no-print"><button className="secondary-button" disabled={!selectedFirm || status.refreshing} onClick={refreshOutstanding} type="button"><RefreshCw size={15} /> {status.refreshing ? 'Refreshing…' : 'Refresh Outstanding'}</button><button className="primary-button" disabled={!report} onClick={exportExcel} type="button"><Download size={15} /> Export Excel</button></div></div>
    <div className="panel firm-wise-filters no-print"><label>Firm<select value={firm} onChange={(event) => { setFirm(event.target.value); setDealingPerson('') }}><option value="">Select Firm</option>{firms.filter((row) => row.name).map((row) => <option key={row.id}>{row.name}</option>)}</select></label><label>As-of Date<input max={today} type="date" value={asOfDate} onChange={(event) => setAsOfDate(event.target.value)} /></label><label>Salesman<select value={dealingPerson} onChange={(event) => setDealingPerson(event.target.value)}><option value="">All Salesmen</option>{people.dealing.map((name) => <option key={name}>{name}</option>)}</select></label></div>
