...
export default function FirmWiseReport({ firms }) {
  ...
  async function exportExcel() {
    ...
    sheet.columns = columns.map(([, label], index) => ({ key: label, width: index === 2 ? 36 : 18 })); for (let row = 5; row <= sheet.rowCount; row += 1) for (let col = 4; col <= 14; col += 1) sheet.getCell(row, col).numFmt = col === 9 ? '0.0"%"' : '#,##0.00'
