...
const columns = [
  ...
  ['invoiceReference', 'Original Invoice'], ['voucherNo', 'Credit Note No.'], ['dealingPerson', 'Salesman'],
  ['month', 'Month'], ['financialYear', 'FY'],
]

export default function CreditNoteReport({ firms }) {
  const today = new Date().toISOString().slice(0, 10)
  const initial = { firm: firms.find((row) => row.name)?.name || '', financialYear: financialYear(today), month: monthName(today), dealingPerson: '', search: '' }
  ...
  const options = report?.options || { firms: [], financialYears: [], months: [], dealingPeople: [] }
  return <section className="stack credit-note-report credit-note-minimal">
    <div className="credit-note-toolbar"><div><strong>{report?.summary.rowCount || 0} entries</strong><span>{number(report?.summary.totalAmount)} total</span>{report?.unmatched.length > 0 && <span title={report.unmatched.join(', ')}>{report.unmatched.length} unmatched</span>}</div><button className="primary-button" disabled={!report?.rows.length} onClick={exportExcel} type="button"><Download size={15} /> Export Excel</button></div>
    <div className="credit-note-filters"><Select label="Firm" value={filters.firm} values={firms.filter((row) => row.name).map((row) => row.name)} onChange={(value) => update('firm', value)} all="All Firms" /><Select label="Financial Year" value={filters.financialYear} values={unique([filters.financialYear, ...options.financialYears])} onChange={(value) => update('financialYear', value)} all="All Years" /><Select label="Month" value={filters.month} values={unique([filters.month, ...options.months], false)} onChange={(value) => update('month', value)} all="All Months" /><Select label="Salesman" value={filters.dealingPerson} values={options.dealingPeople} onChange={(value) => update('dealingPerson', value)} all="All Salesmen" />
